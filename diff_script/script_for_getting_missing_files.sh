...
function get_index_of_paired_file_by_extension() {
  ...
  for (( j=0; j<${#files_from_dir_classes[@]}; j++ )); do
    ...
    current_file_name=$(get_file_name "${files_from_dir_classes[j]}")
    local current_file_name_without_extension
    current_file_name_without_extension=$(get_file_name_without_extension "$current_file_name")
    local current_file_extension
    current_file_extension=$(get_file_extension "$current_file_name")
...
function delete_paired_file_names_from_array__files_from_dir_classes() {
  for (( i=0; i<${#files_from_dir_classes[@]}; i++ )); do
    local file_name
    file_name=$(get_file_name "${files_from_dir_classes[i]}")
...
function get_paired_file_names_for_unpaired_file_names_from_array__files_from_dir_classes() {
  for (( i=0; i<${#files_from_dir_classes[@]}; i++ )); do
    local path_without_file_name
    path_without_file_name=$(get_path_without_file_name "${files_from_dir_classes[i]}")
    local file_name
    file_name=$(get_file_name "${files_from_dir_classes[i]}")
    local file_name_without_extension
    file_name_without_extension=$(get_file_name_without_extension "$file_name")
    ...
    local paired_full_file_name
    paired_full_file_name="${path_without_file_name}${file_name_without_extension}"
    if [[ "$file_extension" == "$extension_cls" ]]; then
...
function get_paired_file_names_for_unpaired_file_names_from_array__files_from_dir_lwc() {
  for (( i=0; i<${#files_from_dir_lwc[@]}; i++ )); do
    local verifiable_file_name
    verifiable_file_name=$(get_file_name_from_dir_lwc "${files_from_dir_lwc[i]}")
    local verifiable_file_name_without_extension
    verifiable_file_name_without_extension=$(get_file_name_without_extension "$verifiable_file_name")
    local verifiable_file_extension
    verifiable_file_extension=$(get_file_extension "$verifiable_file_name")
    local paired_files_indexes=()

    for (( j=0; j<${#files_from_dir_lwc[@]}; j++ )); do
      local current_file_name
      current_file_name=$(get_file_name_from_dir_lwc "${files_from_dir_lwc[j]}")
      local current_file_name_without_extension
      current_file_name_without_extension=$(get_file_name_without_extension "$current_file_name")
      local current_file_extension
      current_file_extension=$(get_file_extension "$current_file_name")
    ...
    if [[ ${#paired_files_indexes[@]} -lt $((max_count_of_files_in_lwc_component_directory-1)) && ${#paired_files_indexes[@]} -gt 0 ]]; then
      ...
      for (( t=0; t<${#copy_paired_files_indexes[@]}; t++ )); do
        local file_name
        file_name=$(get_file_name_from_dir_lwc "${files_from_dir_lwc[copy_paired_files_indexes[t]]}")
        local file_extension
        file_extension=$(get_file_extension "$file_name")
...
printf "\n%s\n" "diff files from dir classes:"
for (( i=0; i<${#files_from_dir_classes[@]}; i++ )); do
  printf "%s = %s\n" "$i" "${files_from_dir_classes[i]}";
done

printf "\n%s\n" "diff files from dir lwc:"
for (( i=0; i<${#files_from_dir_lwc[@]}; i++ )); do
